...
export const HelpCommand = new CommandBuilder()
    .setDescription('Shows the command list or information about a specific command.')
    .setArguments(
        {
            name: 'command',
            required: false
    ...
    .setCallback(async (player, args) => {
        if (args.command) {
            ...
            if (command) {
                const cmdArgs = command.args.map((arg) => `${arg.required ? '<' : '['}${arg.name}${arg.required ? '>' : ']'}`);
        ...
        const message = Object.keys(Commands).map((cmd) => `§e${cmd}§r: ${Commands[cmd].description}`).join('\n');
